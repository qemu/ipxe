...
int image_extract ( struct image *image, const char *name,
	...
	*extracted = alloc_image ( image->uri );
	...
	if ( name ) {
		...
		/* Explicit name provided: use that */
		if ( ( rc = image_set_name ( *extracted, name ) ) != 0 )
			goto err_set_name;

	} else if ( (*extracted)->name ) {

		/* Otherwise, strip any archive or compression suffix */
		if ( ( dot = strrchr ( (*extracted)->name, '.' ) ) != NULL )
			*dot = '\0';
	}
